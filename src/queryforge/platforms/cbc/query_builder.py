...
def _extract_patterns(intent: str, field_map: Dict[str, Dict[str, Any]]) -> Tuple[List[str], List[Tuple[int, int]], List[Dict[str, Any]]]:
    ...
    explicit_patterns = [
        ...
        ("cmdline", _CMDLINE_RE, ["process_cmdline", "parent_cmdline", "childproc_cmdline"]),
...
def build_cbc_query(
    ...
    *,
    ...
    Args:
        schema: CBC schema dictionary
        search_type: Type of search (process_search, binary_search, etc.)
    ...
    if natural_language_intent:
        try:
            ...
            if detected_concepts:
                concept_hints = generate_concept_hints(detected_concepts, "cbc")

                # Add concept-based expressions to the query
                available_fields = _collect_fields(field_map)
                for keyword_category, keywords in concept_hints.items():
                    # Map hint category to CBC field names
                    field_mapping = {
                        "process_keywords": ["process_name", "parent_name", "childproc_name"],
                        "cmdline_keywords": ["process_cmdline", "parent_cmdline", "childproc_cmdline"],
                        "port_keywords": ["netconn_port", "ipport"],
                        "domain_keywords": ["netconn_domain", "domain"],
                        "registry_keywords": ["regmod_name"],
                    }
